...
exports.install = function(opcua) {
    ...
    opcua.Enum = require("node-opcua-enum").Enum;
    ...
    opcua.coreaas.identifiableMap = new Map();
    opcua.coreaas.fetchAASReference = fetchAASReference;
    ...
    function fetchAASReference(ref, callback) {
        assert(ref.coreAASType === "AASReferenceType", "ref is not an AASReferenceType instance.");
        let keys = ref.keys._dataValue.value.value;
        let currentNamespace;
        for (let key of keys) {
            if (opcua.coreaas.identifiableMap.has(key.value)) {
                currentNamespace = opcua.coreaas.identifiableMap.get(key.value);
            }
        ...
        if (typeof currentNamespace == "undefined") 
            return (typeof callback != "undefined") ? callback(new Error("Referred entity not found"), undefined) :  undefined;

        return (typeof callback != "undefined") ? callback(undefined, currentNamespace) : currentNamespace;
